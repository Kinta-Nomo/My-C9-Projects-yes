            ...
            if(makables[i].body.length == creatables[j].length-1){
                ...
                if(match){
                    ...
                    if(gameSlider.discovered.includes(creatables[j][creatables[j].length-1]) == false){
                        gameSlider.discovered.push(creatables[j][creatables[j].length-1])
                    }
                    break
                }
            }
...
function summon(name){
    notations.push(new notation(random(0,500),random(0,500),[random(0,255),random(0,255),random(0,255)],name,n))
...
function slider(width,height){
    ...
    this.draw = function(){
        ...
        for (var i = 0;i<this.discovered.length;i++){
            if (this.discovered[i] != this.picked){
                ...
                strokeWeight(1)
                fill(255);
                ellipse(this.x+(i+1)*(r*2.5),this.height/2,r*2-(r/8))
                ...
                text(this.discovered[i], this.x+(i+1)*(r*2.5)-offsetx,this.height/2+offsety);
    ...
    this.check = function(x,y){
        for (var i = 0;i<this.discovered.length;i++){
            if(around(x,y,this.x+(i+1)*(r*2.5),this.height/2,r-(r/8))){
...
function draw(){
    ...
    for (var i=0;i<notations.length;i++){
        fill(notations[i].color[0],notations[i].color[1],notations[i].color[2])
