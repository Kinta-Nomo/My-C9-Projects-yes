    ...
    if (mouseIsPressed){
        ...
        if (dragging){
            if (previewsPull < (dragStart - mouseX)){
                //left
                if (beginX - (dragStart - mouseX) >= level[selectedGroup]["range"][1]*blockHeight){
                    // level[selectedGroup]["x"] = beginX - (dragStart - mouseX)
                    for (var i=0; i<level[selectedGroup]["blocks"].length; i++){
                        level[selectedGroup]["blocks"][i].x = level[selectedGroup]["blocks"][i].originalX + (level[selectedGroup]["range"][1]*blockHeight)
                    }
                }else if ((beginX - (dragStart - mouseX)) > level[selectedGroup]["range"][0]*blockHeight){
                    level[selectedGroup]["x"] = beginX - (dragStart - mouseX)
                    for (var i=0; i<level[selectedGroup]["blocks"].length; i++){
                        level[selectedGroup]["blocks"][i].x = level[selectedGroup]["blocks"][i].initialX - (dragStart - mouseX)
                    }
                }else{
                    level[selectedGroup]["x"] = level[selectedGroup]["range"][0]*blockHeight
                    for (var i=0; i<level[selectedGroup]["blocks"].length; i++){
                        level[selectedGroup]["blocks"][i].x = level[selectedGroup]["blocks"][i].originalX + level[selectedGroup]["range"][0]*blockHeight
                    }
                }
            }else if(previewsPull > (dragStart - mouseX)){
                //right
                if (beginX - (dragStart - mouseX) <= level[selectedGroup]["range"][0]*blockHeight){
                    // console.log()
                    // level[selectedGroup]["x"] = beginX - (dragStart - mouseX)
                    for (var i=0; i<level[selectedGroup]["blocks"].length; i++){
                        level[selectedGroup]["blocks"][i].x = level[selectedGroup]["blocks"][i].originalX + (level[selectedGroup]["range"][0]*blockHeight)
                    }
                }else if ((beginX - (dragStart - mouseX)) < level[selectedGroup]["range"][1]*blockHeight){
                    level[selectedGroup]["x"] = beginX - (dragStart - mouseX)
                    for (var i=0; i<level[selectedGroup]["blocks"].length; i++){
                        level[selectedGroup]["blocks"][i].x = level[selectedGroup]["blocks"][i].initialX - (dragStart - mouseX)
                    }
                }else{
                    level[selectedGroup]["x"] = level[selectedGroup]["range"][1]*blockHeight
                    for (var i=0; i<level[selectedGroup]["blocks"].length; i++){
                        level[selectedGroup]["blocks"][i].x = level[selectedGroup]["blocks"][i].originalX + (level[selectedGroup]["range"][1]*blockHeight)
                    }
                }
