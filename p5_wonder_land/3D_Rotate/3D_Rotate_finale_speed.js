...
function draw(){
    ...
    projectPoints=rorateObject(newPoints,o,anglex,[0,1])
    projectPoints=rorateObject(projectPoints,o,angley,[0,2])
    projectPoints=rorateObject(projectPoints,o,anglez,[1,2])
    ...
    //if "out", it means the object is behind the observer
    ...
    for (var i = 0;i<orderPoints.length;i++){
        //this is the object of the face
        var faces=orderPoints[i]
        
        //"hha" when point behind obsever
        ...
        //undefined becomes a normal surface
        if (faces.type == undefined){
            
            for (var j = 0;j<faces['coordinate'].length;j++){
                //checks if anyone is behind the observer
                if(faces.coordinate[j][1]<observer[1]){
                    break
                }
                //facePoint is the coordinate
                facePoint=faces['coordinate'][j]
                faces['coordinate'][j] = [getX([facePoint[0],facePoint[1]],[observer[0],observer[1]]),getX([facePoint[2],facePoint[1]],[observer[2],observer[1]])]
            }
            drawFace(faces['coordinate'],faces['color'],0)
            // console.log("hi")
